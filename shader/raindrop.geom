...
void main() {
    ...
    for (int i = 0; i < 30; ++i) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            // high speed low mag, 3.0 is current max vel?
            float mag = 0.05*(3.0-length(pVel[0]));
            float ca = cos(angle);
            float sa = sin(angle);
            float casx = ca * (pVel[0].x + 0.5);
            float sasx = sa + 0.5;
            
            float casy = ca + 0.5;
            float sasy = sa * (pVel[0].y + 0.5);
            
            Position = vec3(
                            inputPosition.x + ca * pSize[0] * ( 1.0 + (snoise(vec2(casx, sasx)) - 0.5)*mag),
                            inputPosition.y + sa * pSize[0] * ( 1.0 + (snoise(vec2(casy, sasy)) - 0.5)*mag),
                            0);
